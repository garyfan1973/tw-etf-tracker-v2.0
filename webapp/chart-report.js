...
(function (root) {
  ...
  function renderStandard(result, { compact = false } = {}) {
    ...
    const position = r.reportMeta?.positionStatus;
    const costCurrency = {TWD: "新台幣", USD: "美元"}[r.reportMeta?.costCurrency] || r.reportMeta?.costCurrency || "";
    const positionLabel = position === "holding" ? `持股者 · 每股平均成本 ${text(r.reportMeta.averageCost)} ${esc(costCurrency)}`
      : position === "watching" ? "空手者" : "";
    ...
    const hero = `<header class="sr-hero"><div class="sr-hero-top"><span class="sr-eyebrow">標的分析 · ${esc(marketName)}</span><span class="sr-overall">${cited(verdict.overall)}</span></div><h2>${text(chart.symbol)} <span>${esc(chart.name || "")}</span></h2><div class="sr-meta"><span>${text(chart.date)}</span><span>${text(chart.timeframe)}</span><span>${text(chart.lastPrice)} ${esc(chart.currency || "")}</span>${positionLabel ? `<span class="sr-position-label">本次設定：${positionLabel}</span>` : ""}</div><p class="sr-verdict">${cited(verdict.state)} · ${cited(verdict.entryNow)}</p><p class="sr-thesis">${cited(verdict.thesis)}</p><div class="sr-risk-banner"><b>最大風險</b><span>${cited(verdict.biggestRisk)}</span></div></header>`;
    ...
    const strategiesHtml = `<div class="sr-subhead"><h4>短中長期操作總結</h4><p>各週期各有失效條件，短單不自動轉為長抱。</p></div><div class="sr-table-wrap"><table class="sr-table sr-strategy-table"><thead><tr><th>週期</th><th>策略方針</th><th>進出場點位參考</th><th>風險控管</th></tr></thead><tbody>${strategyRows}</tbody></table></div><div class="sr-closing"><div class="${position === "holding" ? "sr-personal" : ""}"><b>持股者</b><p>${cited(closing.holder)}</p></div><div class="${position === "watching" ? "sr-personal" : ""}"><b>空手者</b><p>${cited(closing.uninvested)}</p></div></div><div class="sr-watch"><b>追蹤清單</b><ul>${(closing.watchlist || []).map(item => `<li>${cited(item)}</li>`).join("")}</ul></div><div class="sr-final"><b>${cited(verdict.overall)}</b><span>${cited(closing.reason)}</span></div>`;

(function () {
  ...
  function drawChart() {
    ...
    currentRows.forEach((r, i) => {
      ...
      candlePoints.push({ x:cx, y:y((Number(r.open) + Number(r.close)) / 2), date:r.date, row:r });
    ...
    const updateTip = event => {
      ...
      const chartRect = $("chartBox").getBoundingClientRect(), wrapRect = document.querySelector(".kline-chart-wrap").getBoundingClientRect(), tip = $("tip");
      ...
      let leftPos = pointX - wrapRect.left - tipWidth / 2;
      let topPos = pointY - wrapRect.top - tipHeight - 12;
      if (topPos < 8) topPos = pointY - wrapRect.top + 12;

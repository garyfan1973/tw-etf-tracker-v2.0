(function () {
  ...
  function initBonds(data) {
    ...
    let activeKeys=defaultKeys.slice(),visibleKeys=defaultKeys.slice(),range="DEFAULT",view={start:Math.max(0,rows.length-260),end:rows.length};
    function rowsFor(key){return rows.map(row=>({date:row.date,close:row.rates[key]}));}
    function renderMultiSeries(){
      const selected=activeKeys.filter(key=>visibleKeys.includes(key)).map(key=>({key,label:(tenors.find(item=>item.key===key)||{}).label||key,rows:rowsFor(key),color:colors[activeKeys.indexOf(key)%colors.length]})).filter(series=>series.rows.some(row=>Number.isFinite(row.close)));
      ...
      const paths=selected.map(series=>{const path=visibleRows.map((row,i)=>Number.isFinite(row.rates[series.key])?`${i?"L":"M"}${x(i).toFixed(1)},${y(Number(row.rates[series.key])).toFixed(1)}`:"").filter(Boolean).join(" ");return `<path d="${path}" fill="none" stroke="${series.color}" stroke-width="2.4" vector-effect="non-scaling-stroke"/>`;}).join("");
      ...
      $("bondLegend").innerHTML=activeKeys.map((key,index)=>{const label=(tenors.find(item=>item.key===key)||{}).label||key;return `<button class="bond-legend-toggle${visibleKeys.includes(key)?" active":""}" data-bond-key="${esc(key)}" aria-pressed="${visibleKeys.includes(key)}"><i style="--legend:${colors[index%colors.length]}"></i>${esc(label)}</button>`;}).join("");
      $("bondLegend").onclick=event=>{const button=event.target.closest("[data-bond-key]");if(!button)return;const key=button.dataset.bondKey;if(visibleKeys.includes(key)){if(visibleKeys.length===1)return;visibleKeys=visibleKeys.filter(item=>item!==key);}else visibleKeys=[...visibleKeys,key];renderMultiSeries();};
    ...
    function setRange(next){range=next;activeKeys=next==="DEFAULT"?defaultKeys.slice():next==="ALL"?tenors.map(item=>item.key):groups[next].slice();visibleKeys=activeKeys.slice();document.querySelectorAll("#bondRanges [data-bond-range]").forEach(btn=>btn.classList.toggle("active",btn.dataset.bondRange===range));renderMultiSeries();}

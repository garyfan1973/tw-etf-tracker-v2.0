(function () {
  ...
  function initCaptureButton() {
    ...
    button.addEventListener("click", async () => {
      ...
      try {
        const capture = async chartData => ({ chartData, blob:await captureElement(captureTarget) });
        const { chartData, blob } = window.MarketChart?.withAnalysisPreset
          ? await window.MarketChart.withAnalysisPreset(capture)
          : await capture(window.MarketChart?.getAnalysisSnapshot?.() || null);

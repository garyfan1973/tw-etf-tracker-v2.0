(function () {
  ...
  async function init() {
    try {
      ...
      $("channelSections").innerHTML=channels.map(channel=>`<section class="panel video-channel-section${channel.pinned?" is-pinned":""}"><div class="video-section-head"><div><span class="video-kicker">${channel.pinned?"Pinned morning program":"Curated channel"}</span><h2><a href="${esc(channel.url)}" target="_blank" rel="noopener">${esc(channel.name)} <small>↗</small></a>${channel.pinned?'<span class="video-pin">置頂</span>':""}</h2></div><span class="video-count">${(channel.videos||[]).filter(isFresh).length} 部</span></div><div class="video-grid" data-channel-grid="${esc(channel.id)}"></div></section>`).join("");

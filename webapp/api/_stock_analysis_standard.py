...
- JSON 欄位名、程式變數、資料物件名稱、服務代碼只供內部取值，絕不可出現在任何給使用者看的文字欄位。不要寫「以 chartData／adjustedTechnical 為準」或照抄英文欄位；改說「圖表的歷史行情」「除息還原後的技術指標」「系統操作訊號」等有投資意義的名稱。只有確有可靠除息事件且完成還原時，才說明技術趨勢已考慮除息影響；支撐、停損及進出場價位仍以實際報價為準。MA、KD、MACD、RSI 等通用技術指標名稱可以保留。

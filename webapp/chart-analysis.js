...
function renderAnalysis(target, result, compact = false) {
  ...
  hero.append(heading);
  if (result.rating) hero.append(node("strong", "ai-rating", result.rating));
  ...
  const fixedReport = node("section", "ai-fixed-report");
  fixedReport.append(node("div", "ai-fixed-report-label", "新增功能 · 固定格式"));
  const fixedReportBody = node("div");
  fixedReportBody.innerHTML = window.ChartReport.render(result);
  fixedReport.append(fixedReportBody);
  target.append(fixedReport);
}
...
function buildPdfExportFrame() {
  ...
  iframe.title = "PDF 匯出版面";
  iframe.style.cssText = "position:absolute;left:-12000px;top:0;width:1060px;height:100px;border:0;background:#fff";
  iframe.srcdoc = `<!doctype html><html lang="zh-Hant"><head><meta charset="utf-8"><link rel="stylesheet" href="${escapeHtml(new URL('chart-report.css?v=20260907', window.location.href).href)}"><style>
  ...
  ${result.invalidation ? `<div class="invalid"><b>判斷失效條件</b>${escapeHtml(result.invalidation)}</div>` : ""}<section class="fixed"><h2>固定格式分析報告</h2>${window.ChartReport.render(result)}</section><footer>產生時間：${escapeHtml(new Date().toLocaleString("zh-TW"))}。AI 分析不構成投資建議或獲利保證。</footer></body></html>`;
  return new Promise((resolve) => {
    iframe.onload = () => {
      const body = iframe.contentDocument.body;
      iframe.style.height = `${Math.ceil(body.scrollHeight)}px`;

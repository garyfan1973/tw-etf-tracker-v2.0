...
- 限定會員 AI 線圖分析：上傳 JPG／PNG／WebP 線圖，選擇完整、快閃、隔日沖或低接模式；後端校正除息造成的價格落差，依 `stock-analysis-standard` 產生技術、來源化基本面與快閃／短中長期策略。查無可靠資料時明確標示未查證。
- AI 分析結果保存在 Supabase；線圖保存五天，期間可由歷史紀錄重新檢視、匯出 PDF 或寄送 Email，逾期後仍保留文字分析。
- 會員 AI 晨報：最多設定 20 檔台灣／美國股票或 ETF，每日 06:30、07:30、08:30 依實際行情日期補跑盤後技術分析；只有新交易資料才寄送 PDF，且不扣除互動式分析額度。
...
`webapp/skills/stock-analysis-standard/SKILL.md` 是新版線圖分析規範的部署副本，API 在啟動時載入，並以檔案雜湊記錄實際規範版本。模型使用公開網路來源查證基本面；結果須有可核對的來源連結才標示已查證。`vercel.json` 將規範檔納入 Function。會員權限與每日額度維持既有檢查；舊結果仍以原格式顯示。

新結果使用 schema v3，依 `stock-analysis-standard` 提供技術結構、附來源的基本面、快閃及短中長期策略。持倉狀態／成本／幣別為選填，不等於預計買價。結果及規範版本資訊保存於既有 `result` JSON，無需資料庫 migration。

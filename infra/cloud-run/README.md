...
此目錄將重要批次從 GitHub `schedule` 搬到 Cloud Scheduler 與 Cloud Run Jobs。GitHub Actions 的 `workflow_dispatch` 保留為人工備援。四個 Scheduler 定期喚醒；行情批次以 Git 差異判斷是否有新資料，財經內容每日更新四次且只有實質內容變更才提交，晨報則以線圖的實際市場日期與既有寄送紀錄判斷，休市日不重複寄送舊行情，特殊開市或延遲資料可由後續時段補抓、補發。
